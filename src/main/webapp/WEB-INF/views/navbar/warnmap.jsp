...
<link href="${path}/resources/css/common.css?var=3" rel="stylesheet">
  ...
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active fs-4" aria-current="page">Map>비행금지구역</li>
  </ol>
...
<body>
	...
	<div style="border: 1px solid black; float: left; width: 224px;">
			<input type="checkbox" class="chkAir" name="chkBox" value="lt_c_aisprhc" checked/>비행금지구역
			<input type="checkbox" class="chkAir" name="chkBox" value="lt_c_aisctrc" checked/>관제권
			<input type="checkbox" class="chkAir" name="chkBox" value="lt_c_aisresc"/>비행제한구역
			<input type="checkbox" class="chkAir" name="chkBox" value="lt_c_aisdngc"/>위험구역
			<input type="checkbox" class="chkAir" name="chkBox" value="lt_c_aismoac"/>군작전구역
			<input type="checkbox" class="chkAir" name="chkBox" value="lt_c_aisatzc"/>비행장교통구역
			<input type="checkbox" class="chkAir" name="chkBox" value="lt_c_aisuac" checked/>초경량비행장치공역
			<!-- 시범구역은 안보임..... -->
...
<script>
	...
	$(function() {
		...
		$chk.each(function(idx, item) {
			...
			if(item.checked) {
				...
				var imgSrc = "http://api.vworld.kr/req/image?key=" + apiKey + "&service=image&request=GetLegendGraphic&format=png&type=ALL&layer="
	...
	$(document).on('click','.chkAir',function() {
		var $chk = $("input[name='chkBox']");
